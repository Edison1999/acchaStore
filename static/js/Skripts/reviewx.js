(function ($, window, document) {
  ...
  var Paginator = function (element, options) {
    ...
    this.totalPages = Math.max(
      Math.ceil(this.options.totalItems / this.options.perPage),
  ...
  $.fn.paginathing.defaults = {
    ...
    nextText: "&raquo;",
    firstText:
      '<svg style="width: 7px; vertical-align: middle" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="angle-left" class="svg-inline--fa fa-angle-left fa-w-8" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512"><path fill="currentColor" d="M31.7 239l136-136c9.4-9.4 24.6-9.4 33.9 0l22.6 22.6c9.4 9.4 9.4 24.6 0 33.9L127.9 256l96.4 96.4c9.4 9.4 9.4 24.6 0 33.9L201.7 409c-9.4 9.4-24.6 9.4-33.9 0l-136-136c-9.5-9.4-9.5-24.6-.1-34z"></path></svg>',
    lastText:
      ...
      edit_file_frame.on("select", function () {
        ...
        $.each(edit_attachment, function (index, value) {
          if ($("#rx-edit-photo").data("multiple") == true) {
            $(wrapper).prepend(
              '<div class="rx-edit-image"><img src="' +
                value.url +
  ...
  $("#rx-submit").on("click", function () {
    ...
    if (review_title == "") {
      $("#rx-review-title-error").html(ajax.rx_review_title_error);
    } else if (review_text == "") {
      $("#rx_text").focus();
      $("#rx-review-text-error").html(ajax.rx_review_text_error);
    } else {
        ...
        success: function (data) {
          ...
          if (data.success == true) {
            $("#rx-submit")
              .parent()
              .siblings()
              .closest(".rx-form-submit-status")
              .fadeIn()
              .addClass("success")
              .text(ajax.review_success_msg);
            if (data.status == 0) {
              $("#rx-submit")
                .parent()
                .siblings()
                .closest(".rx-form-submit-notice")
                .fadeIn()
                .addClass("rx-notice")
                .html(ajax.review_status_msg);
            }
            setTimeout(function () {
  ...
  $(document).ready(function () {
    ...
    $("#attachmentForm").validate({
      ...
      messages: {
        author: ajax.please_enter_name,
        email: ajax.please_enter_email,
        reviewx_title: ajax.please_enter_title,
        comment: ajax.please_reave_message,
      },
     ...
     *********************************/
    $(document).on("click", "#attachmentForm #submit", function (event) {
      event.preventDefault();
      ...
      if (parseInt($("#enable_recaptcha").val())) {
        ...
        if (reCaptcha.length === 0) {
          ...
          $("#submit")
            .parent()
            .before(
              "<div class='rx-form-submit-status error' style='display:block;'>re-Captcha required!</div>"
            );
          return;
        }
      }
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data.success == true) {
              $("#submit")
                .parent()
                .before(
                  "<div class='rx-form-submit-status success' style='display:block;'>" +
                    ajax.review_success_msg +
                    "</div>"
                );
              if (data.status == 0) {
                $("#submit")
                  .parent()
                  .before(
                    "<div class='rx-form-submit-status rx-notice' style='display:block;'>" +
                      ajax.review_status_msg +
                      "</div>"
                  );
              }
              $("#review_title").val("");
              ...
              $("#submit")
                .parent()
                .before(
                  "<div class='rx-form-submit-status error' style='display:block;'>" +
                    ajax.review_failed_msg +
                    "</div>"
                );
            } else {
              $("#submit")
          ...
          error: function (err) {
            $("#submit")
              .parent()
              .before(
                "<div class='rx-form-submit-status error' style='display:block;'>Sorry something went wrong.</div>"
              );
          },
        });
      }
